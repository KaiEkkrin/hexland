import { MapColouring, QuadtreeMapColouring } from './colouring';
...
test('Surround one square on a square grid', () => {
  const geometry = new SquareGridGeometry(100, 8);
  for (var colouring of [
    new MapColouring(geometry),
    new QuadtreeMapColouring(geometry)
  ]) {
    ...
    // console.log("colourInside: " + colourInside + ", colourOutside: " + [...notSurrounded.map(c => colouring.colourOf(c))]);
...
test('Surround three hexes on a hex grid', () => {
  const geometry = new HexGridGeometry(100, 8);
  for (var colouring of [
    new MapColouring(geometry),
    new QuadtreeMapColouring(geometry)
  ]) {
...
function testSurroundDenseThreeSquare(colouring: IMapColouring, radius: number, step: number) {
  // This test is intended to both confirm the functionality (it should) and
  // exercise the performance of the colouring algorithm.
  // At high density it will be sort of a worst-case scenario for the quadtree algorithm.
...
test('Surround lots of 3-square rooms on a square grid, r=100, s=6', () => {
  testSurroundDenseThreeSquare(new MapColouring(new SquareGridGeometry(75, 12)), 100, 6);
});

test('Surround lots of 3-square rooms on a square grid (quadtree), r=100, s=6', () => {
  testSurroundDenseThreeSquare(new QuadtreeMapColouring(new SquareGridGeometry(75, 12)), 100, 6);
});

test('Surround lots of 3-square rooms on a square grid, r=100, s=60', () => {
  testSurroundDenseThreeSquare(new MapColouring(new SquareGridGeometry(75, 12)), 100, 60);
});

test('Surround lots of 3-square rooms on a square grid (quadtree), r=100, s=60', () => {
  testSurroundDenseThreeSquare(new QuadtreeMapColouring(new SquareGridGeometry(75, 12)), 100, 60);
});