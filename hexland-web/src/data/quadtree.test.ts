...
test('add and sample quadtree dictionary', () => {
  ...
  var s11 = d.sample({ x: -1, y: -1 }); // this will have been merged
  ...
  var s21 = d.sample({ x: -2, y: -1 }); // this will have been merged again
  ...
  var t10 = d.sample({ x: 1, y: 0 }); // changed
  expect(t10?.position.x).toBe(1);
  expect(t10?.position.y).toBe(0);
  expect(t10?.position.size).toBe(1);
  expect(t10?.colour).toBe(4);

  var u72 = d.sample({ x: -7, y: 2 }); // all merged
