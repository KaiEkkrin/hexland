...
Location: [unit/](../hexland-web/unit/)
Framework: Vitest
Configuration: [unit/vitest.config.ts](../hexland-web/unit/vitest.config.ts)
...
1. Add TypeScript interface to [data/feature.ts](../hexland-web/src/data/feature.ts)
2. Implement rendering in [models/three/](../hexland-web/src/models/three/) (e.g., new `*FeatureObject.ts`)
3. Add UI controls in [components/MapControls.tsx](../hexland-web/src/components/MapControls.tsx)
4. Update state machine in [models/mapStateMachine.ts](../hexland-web/src/models/mapStateMachine.ts)
5. Add change tracking support in [models/mapChangeTracker.ts](../hexland-web/src/models/mapChangeTracker.ts)

### Adding a New Firebase Function

1. Add function implementation to [functions/src/index.ts](../hexland-web/functions/src/index.ts) or separate file
2. Export from [functions/src/index.ts](../hexland-web/functions/src/index.ts)
3. Add TypeScript types to [data/](../hexland-web/src/data/) if needed (types are shared between web and functions)
...
2. **Three.js debug helpers**: Enable in [drawing.ts](../hexland-web/src/models/three/drawing.ts) (grid helpers, axes)
...
Output: `hexland-web/build/` directory
...
- Components in [src/components/](../hexland-web/src/components/)
- Top-level page components as `*.tsx` files in [src/](../hexland-web/src/)

### Data Access

- Use `IDataReference<T>` for typed document access
- Use `IDataView<T>` for reactive queries
- Converters in [src/services/converter.ts](../hexland-web/src/services/converter.ts) for Firestore serialization
- All data access through [dataService.ts](../hexland-web/src/services/dataService.ts)

### Map Changes

**CRITICAL**: All map changes must go through the change tracking system.
- Use [mapChangeTracker.ts](../hexland-web/src/models/mapChangeTracker.ts) methods
...
- Location: `hexland-web/firebase-admin-credentials.json`
- **Gitignored** - never commit credentials
- Obtain from Firebase Console: Project Settings → Service Accounts

## CORS Configuration

Firebase Storage CORS configuration in [hexland-web/cors.json](../hexland-web/cors.json).
