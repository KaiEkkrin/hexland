...
**Required**: `hexland-web/firebase-admin-credentials.json`
- **Gitignored** - never commit
- Download from Firebase Console: Project Settings → Service Accounts → Generate New Private Key
- Place in `hexland-web/` directory
- Without this file, Firebase Functions won't work in development

### Emulator Configuration

[firebase.json](../hexland-web/firebase.json) binds all emulators to `0.0.0.0` for Docker compatibility.
...
CORS configuration for Storage bucket in [hexland-web/cors.json](../hexland-web/cors.json).

**If you get CORS errors**:
```bash
gsutil cors set hexland-web/cors.json gs://your-bucket-name.appspot.com
```
...
**Check predeploy hooks** in [firebase.json](../hexland-web/firebase.json):
```json
"functions": {
  "predeploy": [
...
**Solution**:
- Use Node.js 20 (specified in [functions/package.json](../hexland-web/functions/package.json))
...
**Check rewrites** in [firebase.json](../hexland-web/firebase.json):
...
**Cause**: Missing rewrites in [firebase.json](../hexland-web/firebase.json).
...
- Ensure [mapChangeTracker.ts](../hexland-web/src/models/mapChangeTracker.ts) is handling conflicts
...
**Solution**:
- Check Firebase Storage rules in [storage.rules](../hexland-web/storage.rules)
